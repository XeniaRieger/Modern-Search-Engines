...
def search_results(query:str, k:int) -> list[str]:
    ...
    results = []
    if engines["tf_idf"]:
        results.extend(index.retrieve_tfidf(query, k))
    if engines["BM25"]:
...
def save_entry(evalaution_folder:str):
    ...
    engines = [model for model in ["tf_idf", "BM25"] if st.session_state.get(model)]
...
def query_already_exists(evaluation_folder:str, query) ->bool:
    query_name = str(int(hashlib.md5(query.encode('utf-8')).hexdigest(), 16)) + ".pickle" 
    if query_name in  os.listdir(evaluation_folder):
        return True
    return False

def error_return(text:str):
    st.error(text)
    if st.button("Stop"):
...
if __name__ == '__main__':
    ...
    if st.session_state.role == "create_query":
        if st.session_state.subrole == "create":
            pg = st.navigation([st.Page("subpages/CreateQuery.py")])

        if st.session_state.subrole == "rank":
            if query_already_exists(evaluation_folder, st.session_state["query"]):
                error_return("Query already exists")
            else:
                results = search_results(st.session_state["query"], st.session_state["k"])
                if len(results) <= 0:
                    error_return("Query did not return any results")
                else:
                    st.session_state["result"] = results
                    pg = st.navigation([st.Page("subpages/RankQuery.py")])
    ...
    if st.session_state.role == "rerank":
        if st.session_state.subrole == "rerank":
            ...
            pg = st.navigation([st.Page("subpages/Rerank.py")])
    ...
    if st.session_state.role == "evaluate":
        pg = st.navigation([st.Page("subpages/EvaluatePage.py")])

    if st.session_state.role == None:
        pg = st.navigation([st.Page("subpages/start_page.py")])
