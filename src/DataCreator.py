...
def create_folder_structure(path:str) -> str: 
    evaluation_folder = os.path.join(path, "evaluation")
...
def load_document_index():
    return load_index(os.path.join(parent_path, "serialization", "index.pickle"))
...
def search_results(query:str, k:int) -> list[str]:
    ...
    results = []
    if engines["tf_idf"]:
        res =  list(zip(*index.search(query, k)))
        if (len(res)>0):
            results.extend(list(res[0]))
    if engines["BM25"]:
...
def initialize(force:bool=False):
    if st.session_state == {} or force:
        st.session_state.update({
            ...
            "query":None,
            "k":None,
            "result":None,#["Just testing ","Wkipipedia or somthing"],
            "ranking":None,
            #"tf_idf":None,
            #"BM25":None,
            #"LLM":None
