...
with st.form("evaluate"):
    models = st.multiselect("Which retrieval models would you like to evaluate and compare?", ("tf_idf", "BM25", "BM25_divers", "Doc2Query"))
    default_value=10
    with st.container():
        ...
        set_all_val = st.number_input("Set value for all selected", min_value=0, max_value=13, value=10, step=1)
...
if st.session_state.get("metrics"):
    ...
    for m in st.session_state.models:
        ...
        for d in dataset:
            ...
            m_results =  list(zip(*search_results(m, d.query, k)))
            r_time += (time.time() - start_time)

            if (len(m_results)>0):
                model_results = list(m_results[0])#.extend(list(model_results[0]))
            else:
                model_results = []
    ...
    means = df1.groupby(["model"]).mean()
    var = df1.groupby(["model"]).var()

    if st.session_state.metrics.get("MRR"):
        with st.container(height=650, border=True):
            st.subheader(f"MMR@{MRR_val} for retrieval models {models} over {n} queries")
            st.dataframe(means[["MRR"]])
            st.bar_chart(means, y="MRR", use_container_width=False, width=len(models)*150)
    ...
    if st.session_state.metrics.get("NDCG"):
        with st.container(height=650, border=True):
            ...
            st.dataframe(means[["NDCG"]])
            st.bar_chart(means, y="NDCG", use_container_width=False, width=len(models)*150)
    ...
    if st.session_state.metrics.get("MAP"):
        with st.container(height=650, border=True):
            ...
            st.dataframe(means[["MAP"]])
            st.bar_chart(means, y="MAP", use_container_width=False, width=len(models)*150)
